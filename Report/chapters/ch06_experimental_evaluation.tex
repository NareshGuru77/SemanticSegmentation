...
The metrics used for evaluation are Mean Intersection Over Union (mIOU), inference time, number of parameters, floating point operations (FLOPS) and occupied disk memory. 
	\begin{itemize}
		...
		When the ground truth and predictions have no pixels intersecting for a class, the corresponding class IOU will be 0 percent. Similarly, if there is a perfect union between the ground truth and the prediction, the corresponding class IOU will be 100 percent.
...
\label{section:backbones}
	...
	\begin{itemize}
		...
		\item \textbf{Inference from the results}: The results obtained are shown in Figure \ref{Fig:backbones}. Across all the dataset variants, the Xception network backbone achieves higher mIOU than the MobileNetv2 network backbone consistently. Another inference is that the models trained and validated on the white backgrounds dataset achieve slightly higher mIOU than corresponding models trained on the variety of backgrounds dataset because the white backgrounds dataset has fewer variations in terms of backgrounds.
	\end{itemize}
...
\label{section:diffdata}
	...
	The validation set only consists of the real validation images in order to consider only real-world conditions.
		...
		In addition to the existing limitations, the artificial images inherently impose a regularization effect on the training process. This effect can be attributed to the existence of many different backgrounds. On this regard, the existing L2 regularization weight decay term might need to be lowered to enable the model to better fit to the training data.
...
\label{section:indcls}
	...
	\subsection{Confusion matrix}
		\begin{itemize}
			...
			\item \textbf{Expected result}: The variants with the higher number of classes is expected to have more nonleading diagonal terms in the confusion matrix. This expectation is based on the belief that the segmentation model would face difficulties distinguishing objects very similar to each other. This problem is expected to be alleviated by the atWork\_size\_invariant and atWork\_similar\_shapes variants. On the atWork\_binary variant, there is a possibility that a certain percentage of foreground pixels are confused with the background.
